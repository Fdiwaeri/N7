...
for indice_bruit=1:length(tab_Eb_N0_dB)
    ...
    while(nb_erreurs<100)
        ...
        %Mapping_4ASK
        symboles_4ASK=pammod(bits_convertis, M, 0, 'gray');     % A COMPLETER
        %Mapping de 4QAM
        symboles_4QAM=qammod(bits_convertis, M, 'gray');     % A COMPLETER
        ...
        bits_regroupes_4ASK = de2bi(symboles_recus_4ASK, log2(M))';
        bits_regroupes_4QAM = de2bi(symboles_recus_4QAM, log2(M));

        bits_recus_4ASK=reshape(bits_regroupes_4ASK, 1, []);  % A COMPLETER 
        bits_recus_4QAM=reshape(bits_regroupes_4QAM', 1, []);  % A COMPLETER 
