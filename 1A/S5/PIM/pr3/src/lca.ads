...
package LCA is
	...
	generic
		...
		with procedure Afficher_Valeur (Valeur : in T_Valeur);
	procedure Afficher_Debug (Sda : in T_LCA);
		
	function La_Cle (Sda : in T_LCA) return T_Cle;
	
	procedure Inserer(Sda : in out T_LCA; Cle : in T_Cle; Valeur : in T_Valeur; Indice : in Integer);
	
	function Cle_Indice(Sda : in T_LCA; Indice : in Integer) return T_Cle;
	
	function Valeur_Indice(Sda : in T_LCA; Indice : in Integer) return T_Valeur;
	
	procedure Inverser(Sda: in out T_LCA);
	
    procedure Supprimer_premier(Sda: in out T_LCA);
    
    function Suivant(Sda: in T_LCA) return T_LCA;
    
    procedure Remplacer(Sda1 : in T_LCA;Sda2 : in out T_LCA);
    
    -- Les programmes suivants concernent les LCAs dont les Valeurs sont uniques commes les clés.
    function La_Cle_De_Valeur (Sda : in T_LCA; Valeur : in T_Valeur) return T_Cle;
	
	-- Savoir si une Valeur est présente dans une Sda.
	function Valeur_Presente (Sda : in T_LCA ; Valeur : in T_Valeur) return Boolean;
