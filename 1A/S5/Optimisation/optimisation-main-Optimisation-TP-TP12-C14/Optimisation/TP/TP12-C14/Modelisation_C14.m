...
%
    ...
    lambda = beta (2);
    A0 = beta (1);
    J_res = [-exp(-lambda*t), A0*t.*exp(-lambda*t)];
...
function [H_f, r, J] = Hess_f_C14(beta, donnees, residu, J_residu)
%
% Paramètres en entrés
% --------------------
% beta     : vecteur des paramètres
...
%
    ...
    for i = 1 : length(r)
        ri= r(i);
        ti = donnees(i,1);
        H_f = H_f + ri *[0,ti*exp(-lambda*ti);ti*exp(-lambda*ti),-A0*(ti^2)*exp(-lambda*ti)];
    end
