...
public class SimplePDLValidator extends SimplepdlSwitch<Boolean> {
	...
	public Boolean caseProcess(simplepdl.Process object) {
		...
		// Visite des ProcessElement
		for (ProcessElement pe : object.getProcessElements()) {
			this.doSwitch(pe);
		}
		
		// Visite des Resources (non ProcessElement)
		for (simplepdl.Resource r : object.getResources()) {
			this.doSwitch(r);
		}
	...
	public Boolean caseWorkDefinition(WorkDefinition object) {
		...
		for (UsageResource ur : object.getUses()) {
			this.doSwitch(ur);
		}
	 ...
	 * 2 Quantité positive.
     * @param resource élément visité (simplepdl.Resource)
	 * @return résultat de validation (null)
	 */
    @Override
    public Boolean caseResource(simplepdl.Resource resource) {
        ...
        this.result.recordIfFailed(
            quantityIsPositive, 
            resource, 
            "La quantité de la ressource '" + resource.getName() + "' doit être strictement positive (actuel: " + resource.getQuantity() + ")."
        );
        ...
        if (process != null) {
            ...
            // Le nom doit apparaître une seule fois
            if (count > 1) {
    ...
    /**
	 * Méthode appelée lorsque l'objet visité est un UsageResource.
	 * Implante les contraintes statiques sur l'Utilisation des Ressources :
	 * 1 Quantité utilisée positive.
	 * @param usageResource élément visité
	 * @return résultat de validation (null)
	 */
    @Override
    public Boolean caseUsageResource(UsageResource usageResource) {
        
        // Contrainte 1:
        boolean quantityIsPositive = usageResource.getUsageQuantity() > 0;
        this.result.recordIfFailed(
            quantityIsPositive, 
            usageResource, 
            "La quantité de ressource utilisée (usageQuantity) doit être strictement positive (actuel: " + usageResource.getUsageQuantity() + ")."
        );
        
        // Vérification de la validité de la référence de Resource dans le Process
        simplepdl.Resource referencedResource = usageResource.getResource();
        WorkDefinition wd = usageResource.getWorkdefinition();
        
        if (referencedResource != null && wd != null && wd.eContainer() instanceof simplepdl.Process) {
            simplepdl.Process process = (simplepdl.Process) wd.eContainer();
            
            // Vérifie si la ressource référencée est contenue dans la liste des ressources du Process
            boolean resourceInProcess = process.getResources().stream()
                .anyMatch(r -> r.equals(referencedResource));

            if (!resourceInProcess) {
                this.result.recordError(usageResource,
                    "La ressource référencée '" + referencedResource.getName() + 
                    "' n'est pas définie dans la liste des ressources du processus parent."
                );
            }
        }
