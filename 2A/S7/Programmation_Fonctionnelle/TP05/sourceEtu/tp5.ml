...
struct
  type t = int
  let const _ = 0
  let plus e1 e2 = 1 + max e1 e2
  let mult e1 e2 = 1 + max e1 e2
end
...
struct
  type t = string
  let var s = s 
  let def s e1 e2 = "let " ^ (var s) ^ " = " ^ e1 ^ " in " ^ e2
end
...
module PrintExemplesVar = ExemplesVar (Print)
let%test _ = (PrintExemplesVar.exemple3 = "let x = ( 1 + 2 ) in ( x * 3 )")
module PrintExemplesSimples = ExemplesSimples (Print)
let%test _ = (PrintExemplesSimples.exemple1 = "( 1 + ( 2 * 3 ) )")
let%test _ = (PrintExemplesSimples.exemple2 = "( ( 5 + 2 ) * ( 2 * 3 ) )")