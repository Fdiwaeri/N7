...
let coeff_directeur (x1,y1) (x2,y2) = 
   if x1=x2 then failwith "coefficient directeur non défini"
   else (y2-.y1)/.(x2-.x1)
...
let ieme t i =
   ...
   |2, (_,x,_) -> x
   |3, (_,_,x) -> x
   | _ -> failwith "i doit etre entre 1 et 3"
...
let padovan2 n =
   let rec padovan_mem n m l =
      match (n,l) with
      |0,_ -> 0
      |1,_ -> 0
      |_, (x,y,z)-> if n=m then x
      else (padovan_mem n (m+1) (y+z, x, y))

   in padovan_mem n 2 (1,0,0)
...
let rec dragon (xa,ya) (xb,yb) n = 
   ...
   let xc=((xa+xb)/2 + (yb-ya)/2) and yc=((ya+yb)/2 + (xa-xb)/2)
      in
      dragon (xa,ya) (xc,yc) (n-1);
      dragon (xc,yc) (xb,yb) (n-1)
