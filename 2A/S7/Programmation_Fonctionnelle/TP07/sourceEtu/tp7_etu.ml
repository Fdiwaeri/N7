...
module FreeFall (F : Frame) =
  struct
    ...
    let run ((p0, v0) : etat) = 
      ...
      let vit = 
        Flux.map2
          (fun (vx, vy) (dvx, dvy) -> (vx +. dvx, vy +. dvy))
          (Flux.constant v0)
          (integre F.dt acc)
      in
      ...
      let pos =
        Flux.map2
          (fun (px, py) (dpx, dpy) -> (px +. dpx, py +. dpy))
          (Flux.constant p0)
          (integre F.dt vit)
      in
...
let rec unless : 'a Flux.t -> ('a -> bool) -> ('a -> 'a Flux.t) -> 'a Flux.t = 
  fun flux cond f -> Tick (lazy (
  match Flux.uncons flux with
    |None -> None
    |Some (t, q) -> if cond t then Flux.uncons (f t)
    else Some (t, unless q cond f)))
...
module Bouncing (F : Frame) =
  struct
    ...
    let contact_x (x : float) (dx : float) : bool =
      match F.box_x with 
      |(xmin, xmax) -> 
      (x>xmax && dx>0. || x<xmin && dx<0. ) 

    let rebond_x (x : float) (dx : float) : float =
      if contact_x x dx then -.dx else dx

    let contact_y (y : float) (dy : float) : bool =
      match F.box_y with 
      |(ymin, ymax) ->
      (y>ymax && dy>0. || y<ymin && dy<0. ) 

    let rebond_y (y : float) (dy : float) : float =
      if contact_y y dy then -.dy else dy 

    let rec run (etat : etat) : etat Flux.t = 
      let trajectoire = FF.run etat in
      let condition_choc ((x, y), (dx, dy)) =
        contact_x x dx || contact_y y dy
      in
    let reaction ((x, y), (dx, dy)) =
        run ((x,y), (rebond_x x dx, rebond_y y dy))
    in
    unless trajectoire condition_choc reaction
  end
...
module BouncingTest = Bouncing(FTest)
module DrawingTest = Drawing(FTest)
let () = let etat_initial =((400., 800.), (2000.,0.)) in
  DrawingTest.draw(BouncingTest.run etat_initial)
let () = 
  try 
