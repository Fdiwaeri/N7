...
public class Mesh {
    ...
    private Vector[] computeNormals() {
        ...
        for (int i = 0; i < numFaceElements; i += VERTICES_PER_FACE) {
            // Récupération des indices des 3 sommets de la face courante
            int idx1 = faces[i];
            int idx2 = faces[i + 1];
            int idx3 = faces[i + 2];

            // Récupération des vecteurs (coordonnées) correspondants
            Vector v1 = vertices[idx1];
            Vector v2 = vertices[idx2];
            Vector v3 = vertices[idx3];

            // Calcul de deux arêtes du triangle
            Vector edge1 = v2.subtract(v1);
            Vector edge2 = v3.subtract(v1);

            // La normale de la face est le produit vectoriel des deux arêtes
            // On la normalise comme demandé dans l'énoncé
