...
public class TextureShader extends Shader {
    ...
    public void shade(Fragment fragment) {
        ...
        try {
            if (texture != null) {
                // on récupère u et v depuis le fragment
                double u = fragment.getAttribute(START_TEXTURE_ATTRIBUTE);
                double v = fragment.getAttribute(START_TEXTURE_ATTRIBUTE + 1);

                // échantillonnage de la texture à ces coordonnées
                Color texColor = texture.sample(u, v);
                Color finalColor = texColor;

                // Combinaison avec la couleur de base si demandé
                if (combineWithBaseColor) {
                    Color baseColor = fragment.getColor();
                    
                    // On multiplie les composantes de couleur (normalisées de 0 à 1 en divisant par 255.0)
                    int r = (int) ((texColor.getRed() / 255.0) * baseColor.getRed());
                    int g = (int) ((texColor.getGreen() / 255.0) * baseColor.getGreen());
                    int b = (int) ((texColor.getBlue() / 255.0) * baseColor.getBlue());
                    
                    // On s'assure que les valeurs restent bien entre 0 et 255
                ...
                // Affichage du pixel avec le bon type (Color)
                screen.setPixel(fragment.getX(), fragment.getY(), finalColor);
            } else {
                // Fallback de sécurité si la texture n'est pas chargée
