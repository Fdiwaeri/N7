...
public class Rasterizer {
    ...
    public void rasterizeFace(final Fragment v1, final Fragment v2, final Fragment v3)
        ...
        // Bounding Box
        int minX = (int) Math.floor(Math.min(v1.getX(), Math.min(v2.getX(), v3.getX())));
        int maxX = (int) Math.ceil(Math.max(v1.getX(), Math.max(v2.getX(), v3.getX())));
        int minY = (int) Math.floor(Math.min(v1.getY(), Math.min(v2.getY(), v3.getY())));
        int maxY = (int) Math.ceil(Math.max(v1.getY(), Math.max(v2.getY(), v3.getY())));

        // la matrice des coordonnées barycentriques
        ...
        for (int x = minX; x <= maxX; x++) {
            for (int y = minY; y <= maxY; y++) {
                ...
                try {
                    ...
                    Vector baryCoords = C.multiply(p);
                    double alpha = baryCoords.get(0);
                    double beta = baryCoords.get(1);
                    double gamma = baryCoords.get(2);

                    // Test d'inclusion : le pixel est-il dans le triangle ?
                    if (alpha >= -1e-6 && beta >= -1e-6 && gamma >= -1e-6) {
                        // Création du fragment pour le pixel courant
                        Fragment pixelFragment = new Fragment(x, y);

                        // Interpolation linéaire de tous les attributs (couleur, profondeur, normales, UV)
                        int numAttributes = v1.getNumAttributes();
                        for (int i = 0; i < numAttributes; i++) {
                            
                            // Somme pondérée des attributs des 3 sommets
                            double interpolatedValue = alpha * v1.getAttribute(i) +
                                                    beta * v2.getAttribute(i) +
                                                    gamma * v3.getAttribute(i);

                            // Clamper spécifiquement les valeurs de couleur entre 0 et 1
                            if (i == Fragment.COLOR_R || i == Fragment.COLOR_G || i == Fragment.COLOR_B) {
                                interpolatedValue = (double) MathUtils.clamp(interpolatedValue, 0.0, 1.0);
                            }

                            // Assigner l'attribut interpolé au fragment
                            pixelFragment.setAttribute(i, interpolatedValue);
                        }

                        // Affichage du fragment via le shader
                        shader.shade(pixelFragment);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
