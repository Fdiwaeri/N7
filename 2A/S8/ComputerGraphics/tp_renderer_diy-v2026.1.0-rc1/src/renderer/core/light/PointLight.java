...
public class PointLight extends Light {
    ...
    public double getContribution(Vector position, Vector normal, double[] color,
        ...
        double I_diffuse = this.intensity * kd * cosNL;

        // specular contribution = reflet
        double cosHN = Math.max(0.0, normal.dot(h));
        double I_specular = this.intensity * ks * Math.pow(cosHN, s);
