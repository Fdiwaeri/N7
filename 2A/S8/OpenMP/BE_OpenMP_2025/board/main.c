...
void parallel_ops(int n, int nops, int **board, operation_t *operations){
  ...
  #pragma omp parallel private(i,j,operation)
  {
    #pragma omp single
    {
  for(op=0; op<nops; op++){
    ...
    switch (operation.optype) {
    case LEFT:
      #pragma omp task depend(in:board[i][j-1]) depend(out:board[i][j])
      /* printf("%2d  -- LEFT    on %2d %2d   \n",omp_get_thread_num(), i, j); */
      update(&(board[i][j]), board[i][j-1]);
      break;
    case RIGHT:
      ...
      break;
    case UP:
      #pragma omp task depend(in:board[i-1][j]) depend(out:board[i][j])
      /* printf("%2d  -- UP      on %2d %2d   \n",omp_get_thread_num(), i, j); */
      update(&(board[i][j]), board[i-1][j]);
      break;
    case DOWN:
      ...
      break;
    case GATHER:
      #pragma omp task depend(in:board[i][j-1], board[i][j+1], board[i+1][j], board[i-1][j]) depend(out:board[i][j])
      /* printf("%2d  -- GATHER  on %2d %2d   \n",omp_get_thread_num(), i, j); */
      gather(&(board[i][j]), board[i][j-1], board[i][j+1], board[i+1][j], board[i-1][j]);
      break;
    case SCATTER:
      #pragma omp task depend(out:board[i][j-1], board[i][j+1], board[i+1][j], board[i-1][j]) depend(in:board[i][j])
      /* printf("%2d  -- SCATTER on %2d %2d   \n",omp_get_thread_num(), i, j); */
      scatter((board[i][j]), &board[i][j-1], &board[i][j+1], &board[i+1][j], &board[i-1][j]);
      break;
    default:
