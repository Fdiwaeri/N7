...
{
  int i, d;
  double res, scale, ssq, absxi, myscale, myssq;
  double *scales, *ssqs;
  scale = 0.0;
  ssq = 1.0;

  d = omp_get_max_threads();
  scales = (double*) malloc(d * sizeof(double));
  ssqs = (double*) malloc(d * sizeof(double));

#pragma omp parallel private(myscale, myssq, absxi)
{  
  myscale = 0.0;
  myssq = 1.0;
  ...
  scales[omp_get_thread_num()] = myscale;
  ssqs[omp_get_thread_num()] = myssq;
  ...
  for (int i=0;i<d;i++){
    ...
    myssq = ssqs[i];
    if(myscale>scale) {
      ssq = myssq + ssq*pow(scale/myscale,2);
      scale = myscale;
    } else {
      ssq = ssq + myssq*pow(myscale/scale,2);
    }
  }
  res = scale * sqrt(ssq);
