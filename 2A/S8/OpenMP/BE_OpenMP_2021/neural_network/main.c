...
#pragma omp parallel private(l)
{
  ...
  for(l=0; l<L; l++){
    for(i=0; i<N; i++)
      for(j=0; j<N; j++){
        #pragma omp task firstprivate(i,j,l) depend(in:Layers[l].W[i][j], Datas[l].X[j]) depend(out:Datas[l+1].X[i])
        block_mult(Layers[l].W[i][j], Datas[l].X[j], Datas[l+1].X[i], m);
      }
    
    
      for(i=0; i<N; i++)
        #pragma omp task firstprivate(i,l) depend(in:Layers[l].b[i]) depend(inout:Datas[l+1].X[i])
        block_bias_act(Layers[l].b[i], Datas[l+1].X[i], m);
  }
}
}
