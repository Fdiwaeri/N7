...
void pipeline(data *datas, resource *resources, int ndatas, int nsteps){
  ...
  int d, s;
  omp_lock_t *resource_locks;
  resource_locks = (omp_lock_t*) malloc(nsteps*sizeof(omp_lock_t));
  for(int i=0;i<nsteps;i++) omp_init_lock(resource_locks+i);
  ...
  for (d=0; d<ndatas; d++){
    ...
    for (s=0; s<nsteps; s++){
      omp_set_lock(resource_locks+s);
      process_data(datas, d, s, &(resources[s]));
      omp_unset_lock(resource_locks+s);
    }
  }
   
  for(int i=0;i<nsteps;i++) omp_destroy_lock(resource_locks+i);
