...
void work_par_locks(data_t *data, unsigned int *typecounts, int ndata, int ntype){
  int i;
  omp_lock_t *data_locks;
  data_locks = (omp_lock_t*) malloc(ndata*sizeof(omp_lock_t));
  for (i=0;i<ndata;i++) omp_init_lock(data_locks+i);
  #pragma omp parallel for
  for(i=0; i<ndata; i++){
    omp_set_lock(data_locks+i);
    typecounts[analyze(data[i])]++;
    omp_unset_lock(data_locks+i);
  }
  return;
}
